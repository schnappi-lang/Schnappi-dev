...
final class Test extends AnyFunSpec {
  describe("basic") {
    it("works") {
      ...
      assert(infer("(the (pi (withAttrSizeUnknownFinite (makeKind nat)) $_ (withAttrSizeUnknownFinite (makeKind nat))) (lambda $x $x))").isRight)
      assert(infer("(apply (the (pi (withAttrSizeUnknownFinite (makeKind nat)) $_ (withAttrSizeUnknownFinite (makeKind nat))) (lambda $x $x)) zero)").isRight)
    }
  }
}