...
final class Test extends AnyFunSpec {
  describe("basic") {
    it("works") {
      ...
      assert(infer("(apply (the (withAttrLevel (succ zero) (makeKind (pi (withSelfAttrUsageErased (withAttrUsageErased (withAttrLevel (succ zero) (makeKind universe)))) (var t) (pi (withAttrSizeUnknownFinite (makeKind (var t))) (var _) (withAttrSizeUnknownFinite (makeKind (var t))))))) (lambda (var _) (lambda (var x) (var x)))) nat)").isRight)
